...
def detect_analyzer_portin(self):
    try:
        ...
        MeasSystem = CmdFIFO.CmdFIFOServerProxy(
            f"{ipadd}:{self.port_in}", "test_connection", IsDontCareConnection=False
        )
    ...
    try:
        ...
        if not x:
            ...
            self.tab1ExperimentHint.setText(
                " ! Error: Analyzer source '%s' not exist.\n"
...
def detect_analyzer_portin_local():
    try:
        ...
        ipadd = "http://" + host
        MeasSystem = CmdFIFO.CmdFIFOServerProxy(
            f"{ipadd}:{port}", "test_connection", IsDontCareConnection=False
        )
        print(MeasSystem.GetStates())
        return 1
    except:
...
def detect_analyzer_portout_local():
    try:
        ...
        dm_queue = Queue(180)  ## data manager
        listener = Listener(
            dm_queue, host, port, StringPickler.ArbitraryObject, retry=True
        )
